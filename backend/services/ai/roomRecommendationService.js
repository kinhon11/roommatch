...
const buildSearchCriteria = async (message, context = {}, profile = null, intent = 'general') => {
  ...
  const wantsCurrentRoom = intent === 'room_detail'
    || intent === 'compare'
    || /phòng này|tương tự|xung quanh|cùng khu vực/i.test(rawMessage);

  const searchTerms = rawMessage
    .replace(/(?:dưới|tối đa|max|không quá|trên|từ|tối thiểu)\s*\d+(?:[.,]\d+)?\s*(?:triệu|tr|m|m2|m²|k)?/gi, ' ')
    .replace(/\s+/g, ' ')
    .trim();
  ...
  return {
    city,
    priceMin: priceMin || (wantsCurrentRoom && roomPrice !== null ? Math.max(0, Math.floor(roomPrice * 0.75)) : null),
    priceMax: priceMax || (wantsCurrentRoom && roomPrice !== null ? Math.ceil(roomPrice * 1.25) : null),
    areaMin: areaMin || (wantsCurrentRoom && roomArea !== null ? Math.max(0, Math.floor(roomArea * 0.75)) : null),
    areaMax: areaMax || (wantsCurrentRoom && roomArea !== null ? Math.ceil(roomArea * 1.25) : null),
...
const scoreRoomMatch = (room, criteria, profile, context) => {
  ...
  const roomAmenities = (room.amenities || []).map(value => String(value || '').trim().toLowerCase()).filter(Boolean);
  const reasons = [];
  let score = 0;

  if (criteria.city && String(room.city || '').toLowerCase().includes(String(criteria.city).toLowerCase())) {
    score += 30;
    reasons.push('cùng khu vực ' + room.city);
  }

  if (criteria.priceMax && roomPrice !== null && roomPrice <= criteria.priceMax) {
    score += 20;
    reasons.push('giá trong ngưỡng ' + criteria.priceMax.toLocaleString('vi-VN') + ' VNĐ');
  }
  if (criteria.priceMin && roomPrice !== null && roomPrice >= criteria.priceMin) {
    score += 8;
  }

  if (criteria.areaMin && roomArea !== null && roomArea >= criteria.areaMin) score += 5;
  if (criteria.areaMax && roomArea !== null && roomArea <= criteria.areaMax) score += 5;
  ...
  if (criteria.amenities?.length) {
    const wanted = criteria.amenities.map(item => String(item.name || '').trim().toLowerCase());
  ...
  if (profile?.has_profile) {
    ...
    if (preferredCity && String(room.city || '').toLowerCase().includes(String(preferredCity).toLowerCase())) {
      score += 12;
    }

    if (profile.price_stats && roomPrice !== null) {
      const center = profile.price_stats.median || profile.price_stats.average || roomPrice;
    ...
    const preferredAmenities = profile.top_amenities?.map(item => String(item.key || '').trim().toLowerCase()).filter(Boolean) || [];
    const matchedPreferredAmenities = preferredAmenities.filter(name => roomAmenities.includes(name));
    if (matchedPreferredAmenities.length) score += matchedPreferredAmenities.length * 4;
...
const searchRoomsByCriteria = async (criteria, limit = 6, profile = null, context = null) => {
  let query = supabase
    .from('rooms')
    .select('id, title, price, address, city, area, available_slots, created_at, is_available, room_images (id, image_url, is_primary), room_amenities (amenities (id, name, icon))')
    .eq('status', 'approved')
    .eq('is_hidden', false)
  ...
  return rooms
    .map(room => {
      ...
      const scoreResult = scoreRoomMatch({ ...room, amenities }, criteria, profile, context);
